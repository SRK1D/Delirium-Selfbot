...
def loadCommands():
    """Load all of the commands modules in ./commands/
    """
    
    commandPath = "./commands/"
    pythonFiles = [file.replace(".py", "")
                   for file in os.listdir(commandPath)
                   if os.path.isfile(os.path.join(commandPath, file)) and ".py" in file] # Generates a list of python files in the command path and remove the .py
...
settings.settingsInit(";", appSettings["discordToken"]) # Starts the bot (with prefix + token) and creates the commands list
